...
## 代码模版
1. 【状态的定义】dp数组的定义和初始化。
2. 【状态转移方程】dp数组的填充（涉及填充顺序）。

## 适用场景
1. 由于状态不能是很复杂的东西，一般就是一个值。 不能是一个数组，所以不能回溯得到所有可能结果。
2. 适用于**求最值或方案数**。
- 最值：打家劫舍、最大子数组和等。
- 方案数：爬楼梯、目标和。 
## 典型题目
### 一维dp
#### 509-斐波那契数列。
1.一个典型的入门题，题目直接给出了dp数组的定义、初始化和递推公式。 直接for循环填充dp数组即可得到答案。
#### 70-爬楼梯
1. 比第一题的入门题，更进一步。这个题目需要自己分解子问题，从而找出递推公式。而没有直接给出。
#### 746-使用最小花费爬楼梯
1.在第2题的基础上，除了需要自己分解子问题。还需要自己想dp数组的定义。
2.dp数组的定义**和题目是息息相关**的。例如题目问的是“最小花费”而不是“花费”，所以dp数组需要定义为“最小花费”，并且通过min来求解。
3.dp数组的初始化**和题目是息息相关**的。因为题目中爬到第0个台阶、第1个台阶是不需要花费的，因此dp[0]和dp[1]初始化为0。在第0个、第1个台阶向上爬，才需要cost[0]、cost[1]的花费。
#### 343-整数拆分
1.一维dp。但是dp[i]需要for循环遍历才能求出最大值。有点像最长子序列的问题。
2.递推公式容易遗漏条件：dp[i]是一定被拆分过的，k>=2表示至少是2个数的乘积。j*dp[i-j]中的dp[i-j]至少是2个数的乘积，所以j *dp[i-j] 至少是3个数的乘积。忽略了j *（i-j）这种情况。 因此求出的dp[i]不是最大值。
```cpp
class Solution {
public:
...
```
### 背包问题
1. 结题思路：背包问题最难的不是套模版写代码，而是**如何把题目转化为背包问题**。
2. 理论基础-代码模版
- 01背包的二维dp写法，一维dp写法
...
### 01背包
#### 416-分割等和子集
1. 将问题转化为01背包问题。将一个数组的元素恰好分割为sum/2。
2. dp[i][j] 表示将nums[0:i]的元素放入容量为j的背包的最大价值。
- 如果j==sum/2 && dp[i][j]=sum/2，表示恰好可以凑成一个子集，放入容量为sum/2的背包，价值为sum/2。
- 容量为j：表示背包可以装满，可以装不满。也就是可使用的容量是<=j。
- 这个题目中物品的容量和价值，都是nums[i]。 所以本质看的是j==sum/2时，是否可以装满，能装满则为true，不能装满则为false。
3. 解题原则【**真理越辩越清**】
- 不知道状态（dp数组）如何定义，则可以把所有情况都是一遍，看能不能递推。把不能的情况都排除掉。
...
// 典型的0-1背包问题。每个元素可以使用一次。
// 求nums数组的sum。
// 取每个数值，看是否背包是sum/2。
// 状态的定义：dp[i][j] 表示前i个数据，weight和value都是nums[i]
// 状态转移：
// 前i个元素，每个元素只能使用一次，0或1，使用或者不使用。
// 背包的最大weight是sum(nums)

// 【当你不知道使用哪种dp数组的定义时，可以各种都试试，然后排除掉不合适的。真理是越辩越清。】
// dp数组的定义不能是true或false，因为无法递推。
// dp[i]表示前i个数字，能放下的最大value。 这是不对的，因为全部放入，就是最大value。
// dp[i][j]是前i个数字，放在最大容量为value的背包，能放下的最大value。因为可能凑不满。
...
#### 198-打家劫舍1
1. 一维dp。
2. 相邻2个房子不能连续被偷盗。dp[i]的定义是nums[0:i]的最大偷盗数量。没有标志nums[i-1]是否一定被偷盗，所以**nums[i]可否被偷盗，需要分2种情况讨论**。
3. 递推公式：
- nums[i]被偷盗，需要保证nums[i-1]一定不被偷盗：dp[i-2]+nums[i]。
- nums[i]没有被偷盗：dp[i-1]。因为dp[i-1]一定大于等于dp[i-2]，且dp[i-1]会考虑2种情况，也就是说nums[i-1]可能会被偷盗。所以nums[i]一定不能被偷盗。

#### 213-打家劫舍2 
1. 一维dp。
2. 相邻2个房子不能被偷盗。且首尾向量。 决定了nums[0]和nums[n-1]不能同时被偷盗。
- 由于nums[n-1]不能感知nums[0]是否被偷盗，所以**根据nums[0]是否被偷盗，分2种情况讨论**。
- 或者**给dp数组增加一个维度，来标志nums[i]是否被偷盗**。二维dp，本质和分2种情况讨论是一样的。
```cpp
class Solution {
public:
    int rob(vector<int>& nums) {
        ...
        vector<int> dp(n, 0);
        // 情况1：nums[0]偷，nums[n-1]一定不能偷。 
        dp[0] = nums[0];
        dp[1] = max(nums[0], nums[1]);
        for(int i = 2; i < n-1; i++){
            dp[i] = max(dp[i-1], dp[i-2]+nums[i]);
        }
        // 最后一个nums[i]不能偷。 所以dp[n-1]就是一条路径，不是2条路径。
        dp[n-1] = dp[n-2];
        // res的第一种情况
        int res = dp[n-1];

        // 情况2：nums[0]一定不偷, nums[n-1]可以偷。
        // fill(dp.begin(), dp.end(), 0);
        dp[0] = 0;
        dp[1] = nums[1];
        // 此时dp[n-1]的最优解，可以取2种情况。
        for(int i = 2; i < n; i++){
            dp[i] = max(dp[i-1], dp[i-2]+nums[i]);
        }
        // res是取情况1 和情况2的最优解。
...
// 一维dp，在打家劫舍1的基础上，加了数组的首尾想连。
// 方案1：分2种情况讨论
// 由于dp数组的定义中 并没有一个变量来标记nums[i]是否被偷盗，所以需要分2种情况来讨论nums[0]是否被偷盗。
// 如果nums[0]被偷盗了，则nums[n-1]一定不能被偷盗。
// 如果nums[1]没有被偷盗，则dp[n-1]可以取得最优解。

// 方案2：增加dp数组的维度
// 和方案1的分情况讨论 本质上是一样的。 而且dp数组的size会增加一倍。数组的第二维只有0和1两种取值。
// 通过增加dp数组的维度，来标记nums[i]是否被到盗窃。
```
...
1. 用双指针求解即可
#### 115-不同的子序列（子序列的数量）
1. 思路：**有点像背包问题，text1[i]有用和不用，两种情况**。
