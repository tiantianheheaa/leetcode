...
## 算法原理
1. 二分查找是普通查找算法的一种优化算法。普通查找算法是遍历一个数组，时间复杂度是o(n)，二分查找不用遍历所有元素，每次查找将候选区间减少一半，所以时间复杂度是O(log n)。但是二分查找的**前提是有序数组**。
2. 代码：双指针，left和right是要查找的区间的2个端点。if...else...要分2种或3种情况讨论，分别对应1种符合结果的情况 和 2种不符合结果的情况（此时,left++或right--就可以逐渐逼近结果。如果left=mid+1， right=mid-1，就能做到每次二分）。
3. 如果要查找值是否存在：需要[left, right]中的所有值，就while(left<=right)，因为考虑到left==right的情况。 循环退出时，left>right。
4. 如果要查找一个位置/索引：while(left < right)，while循环中不断逼近结果位置，从而while退出时，left == right，也就是要找的位置。 return left或right都可以。
