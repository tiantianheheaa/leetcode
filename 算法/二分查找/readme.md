...
3. 35题考察的是704题目循环退出时的边界。
4. 34题是704题目的多值情况。 也就是有序数组，每个元素可出现多次。找左边界和右边界也很简单，就是找打了就记录更新，然后继续寻找。
5. 69题是二分查找的应用。 将题目抽象为有序数组中值的查找，就可以用二分。
6. 367题-有效的完全平方数，和69题-x的平方根 一样，比69题还简单。
### 704-二分查找（模板）
